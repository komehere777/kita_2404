## Jinja2는 파이썬 기반의 템플릿(정적) 엔진
(html에서 변수 등 여러가지 쓸수있게 해줌)
1. 주석
  {# 주석 내용 #}
...
3. 제어 구조 (%사용)
  ...
  {% endif %}  (if문 끝날때는 endif로 끝남)

  - 시퀀스의 각 항목에 대해 블록의 내용을 반복(for로 시작하면 endfor로 끝남)
  {% for item in items %}
    <p>{{ item }}</p>
  {% endfor %}
  <ul>
...
1. 블록과 확장!!!!
  - 템플릿에서 재정의할 수 있는 블록을 정의(block과 endblock 사이에 콘텐츠를 작성하고, 개별 템플릿 파일에서 재정의할 수 있습니다.)
  {% block 블록이름 %}
  블록 내용
  {% endblock %}
  - 공통 레이아웃을 정의한 기본 템플릿을 확장해주는 것을 다른 html 파일에 표시하고 싶을때 사용
  {% extends "base.html" %}

1. 필터
  - 변수를 출력할 때 필터를 적용하여 값을 변환 (| 파이프 사용)
  {{ 변수명|필터 }}
...
2. block
  {% block title %}{% endblock %} - 블럭의 이름이 title
  설명: block 태그는 템플릿에서 재정의할 수 있는 블록을 정의합니다. block과 endblock 사이의 콘텐츠는 개별 템플릿 파일에서 재정의될 수 있습니다. 각html파일에서 block을 사용하여 다른 내용 들어갈수있다.
...
db 마이그레이션 - 테이블 관련, 제약조건들도 변경되면 마이그레이션 해야함