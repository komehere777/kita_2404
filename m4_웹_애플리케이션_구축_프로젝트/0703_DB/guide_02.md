...
- 기존 코드에서는 Task 모델에 id, title 필드만 있음.
...
- Flask-Migrate 초기 설정: Flask-Migrate를 초기화하려면 애플리케이션 파일을 업데이트하여 Migrate 객체를 생성
    ...
    ```
- 마이그레이션 초기화: flask db init (db없는 경우 최초 한번만 실행)
- 모델 변경 시 마이그레이션 (db있는 경우)
    - 마이그레이션 생성: flask db migrate -m "Add columns to Task" (-m 옵션은 마이그레이션에 대한 설명을 추가하는 옵션)
    - 마이그레이션 적용: flask db upgrade

한국 시간으로 등록일 설정(시스템 시간 가져오기 utc로 가져와서 한국 시간으로 변경하기)

- 수정된 코드에서는 pytz를 사용하여 input_date를 한국 시간으로 설정함.

CSRF 보호(폼 제출 시 CSRF 토큰을 검증하는 방법, 은행 로그인후 다른 사이트에서 접근시 보안이 안되어있으면 공격당할수 있음)
...
폼 렌더링과 폼 제출을 각각 다른 라우트에서 처리(분리)
...
세션 관리(로컬 서버라 중요성을 모르지만 실제 서비스할때 중요함)
- 세션은 웹 애플리케이션에서 클라이언트와 서버 간의 상호작용 상태를 유지하기 위한 방법(모든 동작이 독립적으로 실행, 히스토리가 같은게 없음)
- Flask는 기본적으로 클라이언트 측 세션을 쿠키를 통해 관리합니다. 이를 위해 Flask는 SECRET_KEY를 사용하여 세션 데이터를 암호화하고 서명(쿠키에 저장), (악성코드는 시크릿 키가 없음으로 세션 데이터를 읽을 수 없음)
- 동작 방식
