...
class Task(db.Model):
    ...
    title = db.Column(db.String(200), nullable=False) #nullable=False: 필수값 널 허용안함

#일반적으로 form.py로 따로 빼서 관리하는게 좋음
class TaskForm(FlaskForm):
    title = StringField('Title', validators=[DataRequired()]) #'Title' 레이블, 진자2방식에는 필요
...
def edit(task_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('index'))  #url_for('index') = '/' 이걸로 만들어짐 => def index() 함수실행
